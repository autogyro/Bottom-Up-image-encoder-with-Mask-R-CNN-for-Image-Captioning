...
from fast_rcnn.bbox_transform import bbox_t
...
class ProposalTargetLayer(caffe.Layer):
    ...
    def setup(self, bottom, top):
        ...
        if self._num_attr_classes > 0:
            ...
            ix += 1
        top[ix].reshape(rois_per_image, 81, 28, 28)

        ix += 1

        top[ix].reshape(rois_per_image, 1)
        ix += 1
    ...
    def forward(self, bottom, top):
        ...
        gt_maskes = bottom[2].data
        gt_maskes = gt_maskes.reshape(gt_maskes.shape[0], gt_maskes.shape[1], gt_maskes.shape[2], gt_maskes.shape[3])

        has_mask = bottom[3].data
        has_mask = has_mask.reshape(has_mask.shape[0], has_mask.shape[1])

        coco_category = bottom[4].data
        coco_category = coco_category.reshape(coco_category.shape[0], coco_category.shape[1])
        ...
        labels, rois, bbox_targets, bbox_inside_weights, attributes, relations, mask_target, has_mask_target, coco_target, data_size = _sample_rois(
            all_rois, gt_boxes, fg_rois_per_image,
            rois_per_image, self._num_classes, self._num_attr_classes, 
            self._num_rel_classes, self._ignore_label, gt_maskes, has_mask, coco_category)
        if self._num_attr_classes > 0:
        ...
        top[ix].reshape(*mask_target.shape)
        top[ix].data[...] = mask_target
        ix += 1

        top[ix].reshape(*has_mask_target.shape)
        top[ix].data[...] = has_mask_target
        ix += 1

        top[ix].reshape(*coco_target.shape)
        top[ix].data[...] = coco_target
        ix += 1
...
def _get_bbox_regression_labels(bbox_target_data, num_classes, gt_maskes, has_mask, coco_category):
    ...
    mask_target = np.zeros((clss.size, 81, 28, 28), dtype=np.int32)
    has_mask_target = np.zeros((clss.size, 1), dtype=np.int32)
    coco_target = np.zeros((clss.size, 1), dtype=np.int32)
    ...
    if cfg.TRAIN.AGNOSTIC:
        ...
        for ind in inds:
            ...
            bbox_targets[ind, start:end] = bbox_target_data[ind, 1:]
            has_mask_target[ind, 0] = int(has_mask_target[ind, 0])
            mask_target[ind, :, :, :] = gt_maskes[ind, :, :, :]
            
	    coco_target[ind, 0] = int(coco_target[ind, 0])
            
            bbox_inside_weights[ind, start:end] = cfg.TRAIN.BBOX_INSIDE_WEIGHTS
    return bbox_targets, bbox_inside_weights, mask_target, has_mask_target, coco_target


def _compute_targets(ex_rois, gt_rois, labels, mask_target, coco_category):
    ...
    targets, mask = bbox_t(ex_rois, gt_rois, mask_target, coco_category)
    if cfg.TRAIN.BBOX_NORMALIZE_TARGETS_PRECOMPUTED:
        # Optionally normalize targets by a precomputed mean and stdev
        targets = ((targets - np.array(cfg.TRAIN.BBOX_NORMALIZE_MEANS))
                / np.array(cfg.TRAIN.BBOX_NORMALIZE_STDS))
    return np.hstack(
            (labels[:, np.newaxis], targets)).astype(np.float32, copy=False), mask

def _sample_rois(all_rois, gt_boxes, fg_rois_per_image, rois_per_image, num_classes, 
          num_attr_classes, num_rel_classes, ignore_label, gt_maskes, has_mask, coco_category):
    ...
    bbox_target_data, mask_target = _compute_targets(
        rois[:, 1:5], gt_boxes[gt_assignment[keep_inds], :4], labels, gt_maskes[gt_assignment[keep_inds], :, :, :], coco_category[gt_assignment[keep_inds], :])

    # print 'proposal_target_layer:', bbox_target_data
    bbox_targets, bbox_inside_weights, mask_target2, has_mask_target, coco_target = \
        _get_bbox_regression_labels(bbox_target_data, num_classes, mask_target, has_mask[gt_assignment[keep_inds], :], coco_category[gt_assignment[keep_inds], :])
    
    
    
    has_mask = has_mask[gt_assignment[keep_inds], :]
   
    coco_category = coco_category[gt_assignment[keep_inds], :]
    filter = np.where(has_mask[:32, 0] == 1)[0]
    ...
    print(len(filter))
    bgfilter = filter
    bgfilter = [x+32 for x in bgfilter]
    original_filter = filter
    filter = np.append(filter, bgfilter)
    
    #if len(np.where(has_mask[:, 0] == 1)[0]): 
        #print("original has non-zero mask!")

    labels = labels[filter]
    rois = rois[filter, :]
    bbox_targets = bbox_targets[filter, :]
    bbox_inside_weights = bbox_inside_weights[filter, :]
    mask_target = mask_target[filter, :]
    has_mask = has_mask[filter, :]
    coco_category = coco_category[filter, :]
    return labels, rois, bbox_targets, bbox_inside_weights, attributes, relations, mask_target, has_mask, coco_category, len(filter)
