...
class vg(imdb):
    def __init__(self, version, image_set, ):
        ...
        self._img_path = os.path.join(cfg.DATA_DIR, 'VGdata')
    ...
    def _load_vg_annotation(self, index):
        ...
        coco_id = tree.find('source').find('coco_id').text.lower().strip()
        coco_id_exist = True
        if coco_id == 'none':
            coco_id_exist = False
        ...
        gt_maskes = np.zeros((num_objs, 28, 28), dtype=np.int32)
        has_mask = np.zeros((num_objs), dtype=np.int32)
        coco_category = np.zeros((num_objs), dtype=np.int32)
        ...
        for obj in objs:
            ...
            if obj_name in self._class_to_ind:
                ...
                has_mask[ix] = int(ast.literal_eval(bbox.find('has_mask').text))
                coco_category[ix] = int(bbox.find('coco_category').text)

                if has_mask[ix]:
                    mask = np.asarray(ast.literal_eval(bbox.find('mask').text))
                    gt_maskes[ix, :, :] = mask
