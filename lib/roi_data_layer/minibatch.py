...
def get_minibatch(roidb, num_classes):
    ...
    if cfg.TRAIN.HAS_RPN:
        ...
        gt_maskes = np.zeros((num_gt, 81, 28, 28), dtype=np.int32)
        has_mask = np.zeros((num_gt), dtype=np.int32)
	coco_cats = np.zeros((num_gt), dtype=np.int32)

	coco_cats[:] = roidb[0]['coco_category'][gt_inds]

        has_mask[:] = roidb[0]['has_mask'][gt_inds]
        for i in xrange(num_gt):
            index = gt_inds[i]
            #if not int(coco_cats[index]) == 0:
                #print("giving not 0 mask!")
     	    gt_maskes[i, coco_cats[i], :, :] = roidb[0]['gt_maskes'][index, :, :]
            #gt_maskes[i, :, :, :] = roidb[0]['gt_maskes'][index, :, :]
            #if has_mask[i] == 0:
                #gt_maskes[i, 0, 0, 0] = 1

        #gt_maskes[:, :, :, :] = roidb[0]['gt_maskes'][gt_inds, :]
        ...
        blobs['gt_maskes'] = gt_maskes
        blobs['has_mask'] = has_mask
        blobs['coco_category'] = coco_cats
