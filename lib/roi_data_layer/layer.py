...
class RoIDataLayer(caffe.Layer):
    ...
    def _shuffle_roidb_inds(self, gpu_id=0):
        ...
        if cfg.TRAIN.ASPECT_GROUPING:
            ...
            has_mask = np.array([r['coco_id_exist'] for r in self._roidb])
    ...
    def setup(self, bottom, top):
        ...
        if cfg.TRAIN.HAS_RPN:
            ...
            top[idx] = top[idx].reshape(1, 81, 28, 28)
            self._name_to_top_map['gt_maskes'] = idx
            idx += 1

            top[idx].reshape(1, 1)
            self._name_to_top_map['has_mask'] = idx
            idx += 1

            top[idx].reshape(1, 1)
            self._name_to_top_map['coco_category'] = idx
            idx += 1
