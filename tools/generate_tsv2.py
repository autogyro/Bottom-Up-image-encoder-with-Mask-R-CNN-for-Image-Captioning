...
from fast_rcnn.test import im_detect,_get_blobs, im_detect_mask
...
def load_image_ids(split_name):
    ...
    with open('captions_train2014.json') as f:
      ...
      for item in data['images']:
        ...
        filepath = os.path.join('coco', item['file_name'])
        maskedfilepath = os.path.join('mask', item['file_name'])
        split[image_id] = maskedfilepath
...
def get_detections_from_im(net, net2, im_mask, im_file, image_id, conf_thresh=0.2):
    ...
    pool5a = net.blobs['pool5_flat'].data
    ...
    return {
        ...
        'features': pool5a[keep_boxes]
    }

def mask_generation(net, item, image_ids, conf_thresh=0.2):
    image_id = item['image_id']

    maskedfilepath = image_ids[int(image_id)]

    boxes = str2array(item['boxes'])
    im = cv2.imread(maskedfilepath)
    ...
    item['features'] = base64.b64encode(item['features'] + features2)
    print(item)
    return item
...
def generate_tsv(gpu_id, prototxt, weights, image_ids, outfile):
    ...
    if len(missing) == 0:
        print ('GPU' + str(gpu_id) + 'already completed' + str(len(image_ids)))
    else:
        print ('GPU' + str(gpu_id) + 'missing' + str(len(missing)))
    if len(missing) > 0:
        caffe.set_mode_gpu()
        ...
        with open(outfile, 'ab') as tsvfile:
            ...
            _t = {'misc' : Timer()}
            count = 0
            for im_mask, im_file,image_id in tqdm(image_ids):
                if int(image_id) in missing:
                    _t['misc'].tic()
                    writer.writerow(get_detections_from_im(net, net2, im_mask, im_file, image_id))
                    _t['misc'].toc()
                    count += 1
...
def generate_new_tsv(gpu_id, prototxt, weights, image_ids, outfile):
    tsv = ['out.tsv.0']
    net = caffe.Net("models/vg/ResNet-101/faster_rcnn_end2end_final/test_gt.prototxt", caffe.TEST, weights=weights)
        ...
        for infile in tsv:
            with open(infile) as tsv_in_file:
                ...
                for item in tqdm(reader):
                    try:
                      writer.writerow(mask_generation(net, item, image_ids))
                    except Exception as e:
                      print(e)
...
if __name__ == '__main__':
    ...
    for i,gpu_id in enumerate(gpus):
        ...
        p = Process(target=generate_new_tsv,
                    args=(gpu_id, args.prototxt, args.caffemodel, image_ids, outfile))
        p.daemon = True
        p.start()
        procs.append(p)
    for p in procs:
